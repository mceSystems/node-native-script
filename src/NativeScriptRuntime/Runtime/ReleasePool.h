...
private:
    template <typename U>
    friend void releaseSoon(JSC::JSGlobalObject*, U&&);

    static void releaseSoon(JSC::JSGlobalObject* globalObject, T&& item) {
        NativeScriptRuntime* runtime = NativeScriptRuntime::getRuntime(globalObject);
        ASSERT(!runtime->releasePools().isEmpty());

        ReleasePool<T>* pool = nullptr;

        Item& poolsMap = runtime->releasePools().last();
...
public:
    ReleasePoolHolder(JSC::ExecState* execState) {
        init(execState->lexicalGlobalObject());
    }

    ReleasePoolHolder(JSC::JSGlobalObject* globalObject) {
        init(globalObject);
    }

    ReleasePoolBase::Item relinquish() {
        auto& releasePools = _runtime->releasePools();
    ...
    void drain() {
        if (LIKELY(!_didRelinquish)) {
            ASSERT(!_runtime->releasePools().isEmpty());
            auto& poolsMap = _runtime->releasePools().last();
    ...
    ~ReleasePoolHolder() {
        if (LIKELY(!_didRelinquish)) {
            ASSERT(!_runtime->releasePools().isEmpty());
            _runtime->releasePools().removeLast();
        }
    }

private:
    void init(JSC::JSGlobalObject* globalObject) {
        _runtime = NativeScriptRuntime::getRuntime(globalObject);
        _runtime->releasePools().append(ReleasePoolBase::Item());
    }
...
template <typename T>
void releaseSoon(JSC::JSGlobalObject* globalObject, T&& item) {
    ReleasePool<T>::releaseSoon(globalObject, std::move(item));
}

template <typename T>
void releaseSoon(JSC::ExecState* execState, T&& item) {
    releaseSoon(execState->lexicalGlobalObject(), std::move(item));
}
} // namespace NativeScript
