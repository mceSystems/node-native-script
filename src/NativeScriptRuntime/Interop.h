...
public:
    ...
    static Interop* create(JSC::VM& vm, NativeScriptRuntime* runtime, JSC::Structure* structure) {
        Interop* object = new (NotNull, JSC::allocateCell<Interop>(vm.heap)) Interop(vm, structure);
        object->finishCreation(vm, runtime);
        return object;
    }
...
private:
    ...
    void finishCreation(JSC::VM&, NativeScriptRuntime*);
...
static inline Interop* interop(JSC::ExecState* execState) {
    return NativeScriptRuntime::getRuntime(execState)->interop();
}
} // namespace NativeScript
