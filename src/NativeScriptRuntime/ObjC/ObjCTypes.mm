...
id toObject(ExecState* execState, const JSValue& value) {
    ...
    if (value.isString()) {
        return [[WTFStringToNSString(jsCast<JSString*>(value)->value(execState)) copy] autorelease];
    }
...
JSValue toValue(ExecState* execState, id object, Class klass) {
    ...
    if ([object isKindOfClass:[NSString class]] && klass != [NSMutableString class]) {
        return jsString(execState, CFStringToWTFString((CFStringRef)object));
    }
    ...
    if (class_isMetaClass(object_getClass(object))) {
        return runtime->constructorFor(object_getClass(object), klass);
    }

    return toValue(execState, object, ^{
      return runtime->constructorFor(object_getClass(object), klass)->instancesStructure();
    });
}
...
JSValue toValue(ExecState* execState, id object, Structure* (^structureResolver)()) {
    ...
    if (JSObject* wrapper = NativeScriptRuntime::getRuntime(execState)->objectMap()->get(object)) {
        ASSERT(wrapper->classInfo(vm) != ObjCWrapperObject::info() || jsCast<ObjCWrapperObject*>(wrapper)->wrappedObject() == object);
        return wrapper;
    }

    return ObjCWrapperObject::create(execState->vm(), structureResolver(), object);
}
}
