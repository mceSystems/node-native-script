...
WTF::String ObjCConstructorBase::className(const JSObject* object) {
    NSString * className = [NSStringFromClass(((ObjCConstructorBase*)object)->_klass) stringByAppendingString:@"Constructor"];
    return CFStringToWTFString(reinterpret_cast<CFStringRef>(className));
}
...
const WTF::Vector<WriteBarrier<ObjCConstructorCall>>& ObjCConstructorBase::initializers(VM& vm, JSC::JSGlobalObject* globalObject) {
        ...
        do {
            ...
            for (const Metadata::MethodMeta* method : initializers) {
                if (method->isAvailable()) {
                    ObjCConstructorCall* constructorCall = ObjCConstructorCall::create(vm, globalObject, runtime->objCConstructorCallStructure(), this->_klass, method);
...
EncodedJSValue JSC_HOST_CALL ObjCConstructorBase::constructObjCClass(ExecState* execState) {
    ...
    if (execState->argumentCount() <= 1) {
        ...
        if (initializer && initializer.isCell()) {
            ...
            if (ObjCConstructorNative* nativeConstructor = jsDynamicCast<ObjCConstructorNative*>(vm, constructor)) {
                thisValue = AllocatedPlaceholder::create(vm, nativeConstructor->allocatedPlaceholderStructure(), instance, constructor->instancesStructure());
            } else {
    ...
    for (WriteBarrier<ObjCConstructorCall> initializer : constructor->initializers(vm, execState->lexicalGlobalObject())) {
