...
typedef struct JSBlock {
    ...
    static CFTypeRef createBlock(ExecState* execState, JSCell* function, ObjCBlockType* blockType) {
        ...
        ObjCBlockCallback* blockCallback = ObjCBlockCallback::create(execState->vm(), execState->lexicalGlobalObject(), runtime->objCBlockCallbackStructure(), function, blockType);
    ...
    static void disposeBlock(JSBlock* block) {
        RuntimeLockHolder locker(block->callback->vm());
...
JSValue ObjCBlockType::read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    return ObjCBlockCall::create(execState->vm(), runtime->objCBlockCallStructure(), block, blockType);
}
