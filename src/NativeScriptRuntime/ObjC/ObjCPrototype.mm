...
static EncodedJSValue JSC_HOST_CALL getIterator(ExecState* execState) {
    ...
    ObjCFastEnumerationIterator* iterator = ObjCFastEnumerationIterator::create(execState->vm(), execState->lexicalGlobalObject(), runtime->fastEnumerationIteratorStructure(), object);
...
bool ObjCPrototype::getOwnPropertySlot(JSObject* object, ExecState* execState, PropertyName propertyName, PropertySlot& propertySlot) {
    ...
    if (const MethodMeta* memberMeta = prototype->_metadata->instanceMethod(propertyName.publicName())) {
        ...
        NativeScriptRuntime* runtime = NativeScriptRuntime::getRuntime(execState);
        JSC::VM& vm = *runtime->vm();
        ObjCMethodCall* method = ObjCMethodCall::create(vm, execState->lexicalGlobalObject(), runtime->objCMethodCallStructure(), memberMeta);
        object->putDirect(vm, propertyName, method);
...
void ObjCPrototype::materializeProperties(VM& vm, JSC::JSGlobalObject* globalObject) {
    ...
    for (const PropertyMeta* propertyMeta : properties) {
        if (propertyMeta->isAvailable()) {
            ...
            if (getter)
                descriptor.setGetter(ObjCMethodCall::create(vm, globalObject, runtime->objCMethodCallStructure(), getter));

            if (setter)
                descriptor.setSetter(ObjCMethodCall::create(vm, globalObject, runtime->objCMethodCallStructure(), setter));
