...
static EncodedJSValue callOriginalExtends(ExecState* execState) {
    JSFunction* extends = NativeScriptRuntime::getRuntime(execState)->typeScriptOriginalExtendsFunction();

    CallData callData;
    CallType callType = extends->methodTable(execState->vm())->getCallData(extends, callData);
