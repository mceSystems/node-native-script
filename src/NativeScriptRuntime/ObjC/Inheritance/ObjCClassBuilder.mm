...
static void attachDerivedMachinery(JSC::JSGlobalObject* globalObject, Class newKlass, JSValue superPrototype) {
    ...
    IMP newAllocWithZone = imp_implementationWithBlock(^(id self, NSZone* nsZone) {
      ...
      RuntimeLockHolder lockHolder(vm);
      NativeScriptRuntime* runtime = NativeScriptRuntime::getRuntime(globalObject);

      Structure* instancesStructure = runtime->constructorFor(blockKlass)->instancesStructure();
    ...
    IMP newRetain = imp_implementationWithBlock(^(id self) {
      if ([self retainCount] == 1) {
          if (JSObject* object = NativeScriptRuntime::getRuntime(globalObject)->objectMap()->get(self)) {
              RuntimeLockHolder lockHolder(globalObject->vm());
...
static void addMethodToClass(ExecState* execState, Class klass, JSCell* method, const MethodMeta* meta) {
    ...
    JSCell* returnTypeCell = runtime->typeFactory()->parseType(globalObject, encodings, false);
    const WTF::Vector<JSCell*> parameterTypesCells = runtime->typeFactory()->parseTypes(globalObject, encodings, meta->encodings()->count - 1, false);

    ObjCMethodCallback* callback = ObjCMethodCallback::create(execState->vm(), globalObject, runtime->objCMethodCallbackStructure(), method, returnTypeCell, parameterTypesCells);
    gcProtect(callback);
...
static void addMethodToClass(ExecState* execState, Class klass, JSCell* method, SEL methodName, JSValue& typeEncoding) {
    ...
    NativeScriptRuntime* runtime = NativeScriptRuntime::getRuntime(execState);
    ObjCMethodCallback* callback = ObjCMethodCallback::create(execState->vm(), execState->lexicalGlobalObject(), runtime->objCMethodCallbackStructure(), method, returnTypeValue.asCell(), parameterTypesCells);
    gcProtect(callback);
...
void ObjCClassBuilder::addProperty(ExecState* execState, const Identifier& name, const PropertyDescriptor& propertyDescriptor) {
    ...
    if (propertyMeta) {
        ...
        if (const MethodMeta* getter = propertyMeta->getter()) {
            ...
            const WTF::Vector<JSCell*> parameterTypes = runtime->typeFactory()->parseTypes(globalObject, encodings, getter->encodings()->count - 1, false);

            ObjCMethodCallback* getterCallback = ObjCMethodCallback::create(vm, globalObject, runtime->objCMethodCallbackStructure(), propertyDescriptor.getter().asCell(), returnType, parameterTypes);
        ...
        if (const MethodMeta* setter = propertyMeta->setter()) {
            ...
            JSCell* returnType = runtime->typeFactory()->parseType(globalObject, encodings, false);
            const WTF::Vector<JSCell*> parameterTypes = runtime->typeFactory()->parseTypes(globalObject, encodings, setter->encodings()->count - 1, false);

            ObjCMethodCallback* setterCallback = ObjCMethodCallback::create(vm, globalObject, runtime->objCMethodCallbackStructure(), propertyDescriptor.setter().asCell(), returnType, parameterTypes);
...
void ObjCClassBuilder::addInstanceMembers(ExecState* execState, JSObject* instanceMethods, JSValue exposedMethods) {
    ...
    if (instanceMethods->hasOwnProperty(execState, execState->vm().propertyNames->iteratorSymbol)) {
        ...
        IMP imp = imp_implementationWithBlock(^NSUInteger(id self, NSFastEnumerationState* state, id buffer[], NSUInteger length) {
          return TNSFastEnumerationAdapter(self, state, buffer, length, execState->lexicalGlobalObject());
        });
...
ObjCConstructorDerived* ObjCClassBuilder::build(ExecState* execState) {
    ...
    runtime->_objCConstructors.insert({ klass, Strong<ObjCConstructorBase>(execState->vm(), this->_constructor.get()) });
    attachDerivedMachinery(execState->lexicalGlobalObject(), klass, this->_baseConstructor->get(execState, runtime->vm()->propertyNames->prototype));
