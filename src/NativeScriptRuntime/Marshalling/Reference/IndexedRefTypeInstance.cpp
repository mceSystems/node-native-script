...
JSValue IndexedRefTypeInstance::read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    return IndexedRefInstance::create(execState->vm(), execState->lexicalGlobalObject(), runtime->interop()->indexedRefInstanceStructure(), referenceType->innerType(), pointer);
}
