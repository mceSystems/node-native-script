...
EncodedJSValue JSC_HOST_CALL ReferenceConstructor::constructReference(ExecState* execState) {
    ...
    if (tryGetFFITypeMethodTable(vm, maybeType, &ffiTypeMethodTable)) {
        ...
        PointerInstance* pointer = jsCast<PointerInstance*>(runtime->interop()->pointerInstanceForPointer(vm, handle));
        pointer->setAdopted(adopted);
        result = ReferenceInstance::create(vm, execState->lexicalGlobalObject(), runtime->interop()->referenceInstanceStructure(), maybeType.asCell(), pointer);
        ...
        result = ReferenceInstance::create(vm, runtime->interop()->referenceInstanceStructure(), maybeType);
    }
