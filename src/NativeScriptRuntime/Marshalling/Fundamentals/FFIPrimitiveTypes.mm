...
static JSValue cStringType_read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    JSCell* type = runtime->typeFactory()->int8Type();
    PointerInstance* pointer = jsCast<PointerInstance*>(runtime->interop()->pointerInstanceForPointer(execState->vm(), const_cast<char*>(string)));
    return ReferenceInstance::create(execState->vm(), execState->lexicalGlobalObject(), runtime->interop()->referenceInstanceStructure(), type, pointer);
}
