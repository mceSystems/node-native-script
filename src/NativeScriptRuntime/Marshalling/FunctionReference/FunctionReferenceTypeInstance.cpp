...
JSValue FunctionReferenceTypeInstance::read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    return FFIFunctionCall::create(execState->vm(), runtime->ffiFunctionCallStructure(), functionPointer, WTF::emptyString(), functionReferenceType->returnType(), functionReferenceType->parameterTypes(), false);
}
...
void FunctionReferenceTypeInstance::write(ExecState* execState, const JSValue& value, void* buffer, JSCell* self) {
    ...
    if (FunctionReferenceInstance* functionReference = jsDynamicCast<FunctionReferenceInstance*>(execState->vm(), value)) {
        if (!functionReference->functionPointer()) {
            NativeScriptRuntime* runtime = NativeScriptRuntime::getRuntime(execState);
            FunctionReferenceTypeInstance* functionReferenceType = jsCast<FunctionReferenceTypeInstance*>(self);
            FFIFunctionCallback* functionCallback = FFIFunctionCallback::create(execState->vm(), execState->lexicalGlobalObject(), runtime->ffiFunctionCallbackStructure(), functionReference->function(), functionReferenceType);
