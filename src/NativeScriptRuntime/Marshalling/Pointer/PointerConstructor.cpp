...
EncodedJSValue JSC_HOST_CALL PointerConstructor::constructPointerInstance(ExecState* execState) {
    ...
    JSValue result = NativeScriptRuntime::getRuntime(execState)->interop()->pointerInstanceForPointer(execState->vm(), value);
...
JSValue PointerConstructor::read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    return NativeScriptRuntime::getRuntime(execState)->interop()->pointerInstanceForPointer(execState->vm(), const_cast<void*>(data));
}
