...
JSValue RecordConstructor::read(ExecState* execState, const void* buffer, JSCell* self) {
    ...
    PointerInstance* pointer = jsCast<PointerInstance*>(runtime->interop()->pointerInstanceForPointer(execState->vm(), data));
    pointer->setAdopted(true);
    RecordInstance* record = RecordInstance::create(execState->vm(), execState->lexicalGlobalObject(), constructor->instancesStructure(), size, pointer);
    return record;
}
...
EncodedJSValue JSC_HOST_CALL RecordConstructor::constructRecordInstance(ExecState* execState) {
    ...
    RecordInstance* instance = RecordInstance::create(execState->vm(), execState->lexicalGlobalObject(), constructor->instancesStructure(), ffiType->size, pointer);
